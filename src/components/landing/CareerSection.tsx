...
function CareerRow({ entry, index }: { entry: ExperienceEntry; index: number }) {
  ...
  return (
    <motion.li
      className={`group border-b border-border/40 transition-colors duration-300 hover:bg-accent/20 ${
    ...
    >
      <div className="flex flex-col gap-1.5 py-5 md:flex-row md:items-baseline md:gap-8 md:py-6">
        <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground/70 md:w-44 md:shrink-0">
          {entry.period}
        </span>
        <div className="flex-1">
          ...
          <p
            className={`mt-1 text-sm ${isPrimary ? 'text-accent/80' : 'text-muted-foreground'}`}
          >
...
export function CareerSection(): React.JSX.Element {
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-6 py-16 md:px-10 md:py-24">
      <motion.h2
        className="font-display mb-8 text-2xl font-bold tracking-tight text-foreground md:text-3xl"
