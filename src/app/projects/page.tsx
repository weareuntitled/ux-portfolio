...
import { getProjectDefaults, getProjects, resolveProject } from '@/lib/cms/payload';
...
export default async function ProjectsListPage() {
  ...
  const [defaults, projects, enterpriseProjects] = await Promise.all([
    getProjectDefaults({ draftMode: isDraft }),
    getProjects({ draftMode: isDraft }),
    getEnterpriseProjectsForNav({ draftMode: isDraft }),
  ]);

  const resolvedProjects = projects.map((project) => resolveProject(defaults, project));
